...
async function getList() {
    ...
    showLoader();
    let res=await axios.get("/allEvent");
    hideLoader();
    ...
    res.data.forEach(function (event,index) {
        let row=`<tr>
            ...
            <td>
            <p class="text-xs font-weight-bold mb-0"> ${event['time']}</p>
            </td>
            ...
            <td class="align-middle">
                <button data-id="${event['id']}" class="btn editBtn btn-sm btn-outline-success">Edit</button>
                <button data-id="${event['id']}" class="btn deleteBtn btn-sm btn-outline-danger">Delete</button>
            </td>
        </tr>`
